...
export function validateFeeItemRow(row, rowIndex) {
    const errors = [];
...
export function validateInvoiceRow(row, rowIndex) {
    const errors = [];
 ...
 */
export function generateFeeItemsCsv(feeItems, selectedFields, classes, terms) {
 ...
 */
export function generateInvoicesCsv(invoices, selectedFields, classes, terms, students) {
    const activeFields = selectedFields.filter(f => f.selected);
    const headers = activeFields.map(f => f.label);
 ...
 */
export function autoDetectMapping(csvHeaders, type) {
    const targetFields = type === 'fees' ? FEE_ITEM_FIELDS : INVOICE_FIELDS;
    const mappings = [];
    csvHeaders.forEach(header => {
        const normalized = header.toLowerCase().trim();
        // Try exact match first
        let match = targetFields.find(f => f.label.toLowerCase() === normalized ||
            String(f.key).toLowerCase() === normalized);
        // Try partial match
        if (!match) {
            match = targetFields.find(f => normalized.includes(f.label.toLowerCase()) ||
 ...
 */
export function transformDataWithMapping(data, mappings) {
    return data.map(row => {
        const transformed = {};
 ...
 */
export function displayRowToDataIndex(displayRow) {
 ...
 */
export function matchCsvColumns(csvHeaders, expectedColumns) {
 ...
 */
export function parseFlexibleCsv(csvText, expectedColumns) {
 ...
 */
export function findColumnByVariations(headers, variations) {
