import type { TimetableEntry, Subject } from '../types.js';

export interface TimetableCandidate extends Partial<TimetableEntry> {
    day_of_week: string;
    period_id: number;
...
interface SchedulingParams {
    ...
    candidateEntry: TimetableCandidate;
    subjects: Subject[];
    dryRun?: boolean;
}

export interface SchedulingResult {
    entriesToUpsert: TimetableCandidate[];
...
function getSubjectConfig(subjectId: number, subjects: Subject[]): SubjectConfig {
...
function pickHighest(entries: { entry: TimetableEntry; meta: SubjectConfig }[]): { entry: TimetableEntry; meta: SubjectConfig } | null {
...
function hasTeacherConflict(existingEntries: TimetableEntry[], candidate: TimetableCandidate) {
...
function hasLocationConflict(existingEntries: TimetableEntry[], candidate: TimetableCandidate) {
    if (!candidate.location_id) return null;
 ...
 */
export function applySchedulingRules(params: SchedulingParams): SchedulingResult {
    const { existingEntries, candidateEntry, subjects } = params;
    const baseResult: SchedulingResult = { entriesToUpsert: [], entriesToDelete: [] };
    ...
    const slotEntries = existingEntries.filter(
        (e) =>
            e.day_of_week === candidateEntry.day_of_week &&
    ...
    if (candidateMeta.isSolo) {
        if (slotEntriesWithMeta.length > 0) {
            ...
            if (candidateMeta.priority > highestExisting.meta.priority) {
                baseResult.entriesToDelete.push(...slotEntriesWithMeta.map(({ entry }) => entry.id).filter(Boolean) as number[]);
            } else {
    ...
    if (blockingEntries.length > 0) {
        ...
        if (candidateMeta.canCoRun) {
            ...
            if (candidateMeta.priority > strongestBlocking.meta.priority) {
                baseResult.entriesToDelete.push(...blockingEntries.map(({ entry }) => entry.id).filter(Boolean) as number[]);
            } else {
...
export function canAddCoRunningSubject(existingEntries: TimetableEntry[], candidate: TimetableCandidate, subjects: Subject[]) {
