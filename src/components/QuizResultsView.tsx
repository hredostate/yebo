...
const QuizResultsView: React.FC<QuizResultsViewProps> = ({ quiz, onBack, addToast, scoreEntries = [], attendanceRecords = [], reports = [] }) => {
    ...
    const handleSummarizeShortAnswers = async (question: any) => {
        ...
        try {
            ...
            console.log('[AI] QuizResultsView: Starting summary', { model: getCurrentModel() });
            ...
            console.log('[AI] QuizResultsView: Summary completed', { success: true });
            setSummaries(prev => ({ ...prev, [question.question_id]: textFromAI(response) }));
        } catch (e: any) {
            console.error('[AI] QuizResultsView: Summary failed', { error: e.message });
