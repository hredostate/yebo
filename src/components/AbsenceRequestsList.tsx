...
const AbsenceRequestsList: React.FC<AbsenceRequestsListProps> = ({
  ...
  const getStatusBadge = (status: AbsenceRequestStatus) => {
    const badges = {
      pending: {
        icon: ClockIcon,
        class: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300 border border-amber-300 dark:border-amber-700',
        label: 'Pending'
      },
      approved: {
        icon: CheckCircleIcon,
        class: 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300 border border-green-300 dark:border-green-700',
        label: 'Approved'
      },
      denied: {
        icon: XCircleIcon,
        class: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300 border border-red-300 dark:border-red-700',
        label: 'Denied'
      }
    };
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex-1">
          <input
            ...
            className="w-full px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>

        {/* Status Filter */}
        <div className="flex items-center gap-2">
          <FilterIcon className="h-5 w-5 text-slate-500 dark:text-slate-400" />
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value as AbsenceRequestStatus | 'all')}
            className="px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
      ...
      {filteredRequests.length === 0 ? (
        <div className="text-center py-12">
          <ClockIcon className="h-12 w-12 text-slate-400 mx-auto mb-3" />
          <p className="text-slate-500 dark:text-slate-400">No absence requests found</p>
          {searchQuery && (
            <button
              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium text-sm transition-colors shadow-md flex items-center gap-2"
                  >
