      ...
      if (userProfile && userType === 'staff') {
          try {
            ...
            const payrollAdjustmentsQuery = hasPayrollManagePermission
                ...
                : supabase.from('payroll_adjustments').select('*, user:user_profiles(name)').eq('user_id', userProfile.id);
            ...
            // Student Data Fetching
             try {
                 ...
                 const results = await Promise.allSettled([
                     supabase.from('student_term_reports').select('*, term:terms(*)').eq('student_id', studentProfile.student_record_id),
        ...
        handleCreateClassAssignment: async (assign: any, group: any) => {
             ...
             if (!academicClass) {
                 ...
                 if (verifyError || !dbAcademicClass) {
                     console.warn('[handleCreateClassAssignment] Academic class not found in database, refreshing data...', {
                         academicClassId: academicClass.id,
                         error: verifyError
                     });
                     // Refresh data and notify user to retry
                     await fetchData();
                     addToast('Data was out of sync. Please try again.', 'warning');
                     return false;
                 }
