...
const students: Student[] = [
  ...
  { id: 6, name: 'Fola', campus_id: 1, school_id: 1, grade: '', admission_number: 'F1', class_id: 0, arm_id: 0, status: StudentStatus.Active, reward_points: 0 }
];
...
const scoreEntries: ScoreEntry[] = [
  ...
  { id: 3, student_id: 4, term_id: 1, academic_class_id: 101, subject_name: 'English', total_score: 70, grade_label: 'B', school_id: 1 }
];
...
const broadScope = { termId: 1, sessionLabel: '2024/2025' } as const;
const broadIssues = findIntegrityIssues(reports, enrollments, students, scoreEntries, broadScope, academicClasses);
assert.ok(!broadIssues.some(i => i.message.includes('Fola')), 'Students unrelated to scoped data should not be flagged when campus is unspecified');
