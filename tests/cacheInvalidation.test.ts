...
assert.ok(
  dedupedInsert && dedupedInsert.filter((i: Item) => i.id === 1).length === 1,
  'Insert should dedupe by id',
);
...
assert.ok(updated && updated.find((i: Item) => i.id === 2)?.name === 'Beta Updated', 'Update should merge payload');

// Update should add missing items if not found
const upserted = applyCacheMutation(updated, { type: 'update', record: { id: 4, name: 'Delta' } });
assert.ok(upserted && upserted.some((i: Item) => i.id === 4), 'Update should upsert when id not found');

// Delete should remove matching id (from match)
const deleted = applyCacheMutation(upserted, { type: 'delete', match: { id: 1 } });
assert.ok(deleted && !deleted.some((i: Item) => i.id === 1), 'Delete should remove item by id');
